...
const BlockContentSchema_v1 = z.discriminatedUnion("type", [
  ...
  z.object({
    ...
    path: z.string(),
    uploadStatus: z.union([
      z.literal("notUploaded"),
      z.literal("uploading"),
      z.literal("uploaded"),
    ]),
...
const BlockSchema_v1 = z.discriminatedUnion("type", [
  NormalBlockSchema_v1,
  MirrorBlockSchema_v1,
  VirtualBlockSchema_v1,
]);
...
export const parseV1Backup = (text: string) => {
  ...
  const fromV1Block = (v1Block: z.infer<typeof BlockSchema_v1>): MinimalBlock | null => {
    const params = {
      ...
      ...(v1Block.type === "normalBlock"
        ? {
            content:
              v1Block.content.type === "text"
                ? [BLOCK_CONTENT_TYPES.TEXT, v1Block.content.docContent]
                : v1Block.content.type === "code"
                  ? [BLOCK_CONTENT_TYPES.CODE, v1Block.content.code, v1Block.content.lang]
                  : v1Block.content.type == "image"
                    ? [
                        BLOCK_CONTENT_TYPES.IMAGE,
                        v1Block.content.path,
                        v1Block.content.align,
                        v1Block.content.caption,
                        v1Block.content.width,
                      ]
  ...
  const blocks = result.data.blocks.map(fromV1Block).filter((b) => !!b);
  return blocks;
};
