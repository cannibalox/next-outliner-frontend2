  ...
  routes: [
    ...
    },
    {
      ...
      beforeEnter: (to, from, next) => {
        const tokenContext = globalThis.getTokenContext();
        const tokenPayload = tokenContext?.tokenPayload?.value;
        const targetServerUrl = to.params.serverUrl as string;
        const targetLocation = to.params.location as string;
        // invalid or missing token
        if (!tokenPayload || tokenPayload.serverUrl !== targetServerUrl || tokenPayload.location !== targetLocation) {
